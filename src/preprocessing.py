...
def load_artifacts():
    ...
    artifacts = {
        ...
        "model": joblib.load(
            artifacts_dir / "notebook_06" / "final_model.joblib"
        ),
    }
...
def apply_frequency_encoding(
    ...
    df_encoded['city_frequency'] = city_encoder.transform(
        df_encoded['customer_city']
    ).ravel()
    ...
    df_encoded = df_encoded.drop(
        columns=['customer_city', 'customer_zip_code_prefix']
    )
